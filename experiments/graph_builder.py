...
class ASTBuilder(IdentityDagWalker):
    ...
    def walk(self, formula, **kwargs):
        # formula has been traveled already
        ...
        res = self.iter_walk(formula, **kwargs)  # iterative walk of dag
    ...
    def add_node(self, formula):
        # support all 66 valid operators in .smt2 file
...
def main(parser):
    ...
    parts = file.split('/')
    assert len(parts) > 3, SyntaxError
    output_filename = parts[2]

    np.savez_compressed(file[:-5] + ".npz", nodes=nodes, edges=edges, edge_attr=edge_attr)
    # np.savez_compressed(output_filename+".npz", nodes=nodes, edges=edges, edge_attr=edge_attr)
...
if __name__ == "__main__":
    ...
    # parser.add_argument('output_file', help='the prefix of output file')
    main(parser=parser)
