...
Help()
{
   ...
   echo "example command of coverting .smt2 instances in sage2 dataset to .npz file:"
   echo "./build_graphs -d "dataset_fastsmt/sage2/all" -j 8"
}
...
if [ $Parallel -gt 1 ] ; then
   ...
   then  
      ...
      find $Directory -name "*.smt2" | parallel --bar -j $Parallel python3 graph_builder.py
   fi
else
  find $Directory -name "*.smt2" | xargs -I '{}' python3 graph_builder.py '{}'
fi
